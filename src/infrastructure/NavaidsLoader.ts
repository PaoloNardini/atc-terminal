...
const loadWaypoints = async (context: Context, minCoordinates: Coordinate, maxCoordinates: Coordinate): Promise<Record<string, Waypoint>> => {
    debug(`Load Waypoints between ${minCoordinates.latitude}/${minCoordinates.longitude} and ${maxCoordinates.latitude}/${maxCoordinates.longitude}`);
    const waypoints: Record<string, Waypoint> = {}
    if (!!!minCoordinates || !!!maxCoordinates) {
        return waypoints
    }
    const data = fs.readFileSync(path.join(__dirname, '../../data/1712/Waypoints.txt'), 'utf8')
    if (data) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (words[3] != undefined) {
                ...
                if (latitude >= minCoordinates.getLatitude()
                    ...
                    let w: Waypoint | undefined = findWaypoint(context, waypoint_name, coordinate)
                    if (!w) {
...
export const findWaypoint = (context: Context, name: string, coordinates?: Coordinate): Waypoint | undefined => {
    ...
    if (w) {
        if (coordinates) {
            if ((Math.abs(w.latitude - coordinates.getLatitude()) < 0.05) && (Math.abs(w.longitude - coordinates.getLongitude()) < 0.05)) {
...
const loadNavaids = async ( context: Context, minCoordinates: Coordinate, maxCoordinates: Coordinate): Promise<void> => {
    debug( `Load Navaidsscenario`)
    const data = fs.readFileSync(path.join(__dirname, '../../data/1712/Navaids.txt'), 'utf8')
    if (!!!minCoordinates || !!!maxCoordinates) {
        return
    }
    if (data) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (words.length > 9) {
                ...
                if (latitude >= minCoordinates.getLatitude() 
                    ...
                    let navaid: Waypoint | undefined = findWaypoint(context, navaid_label, new Coordinate(latitude, longitude))
                    if (!navaid) {
                        navaid = addWaypoint(context, navaid_label, new Coordinate(latitude, longitude))
                        navaid.freq = freq;
                    }
                    ...
                    if (navaid_name.includes('ILS/CAT III')) {
                        ...
                        if (navaid.navaidType == NavaidType.NAVAID_TYPE_NDB && update == true) {
                        ...
                        if (navaid.navaidType == NavaidType.NAVAID_TYPE_VORDME && update == true) {
                            navaid.navaidType = NavaidType.NAVAID_TYPE_VORDMENDB;
                        }
                        else {
