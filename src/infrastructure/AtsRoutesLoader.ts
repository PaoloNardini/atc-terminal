...
export const makeAtsRoutesGateway = (context: Context): AtsRoutesGateway => {
    ...
    return {
        async loadAtsRoutesByCoordinates( minCoordinates, maxCoordinates) {
            const atsRoutes = await loadAtsRoutes(context, minCoordinates, maxCoordinates)
...
const loadAtsRoutes = async (context: Context, minCoordinates: Coordinate, maxCoordinates: Coordinate): Promise<AtsRoute[]> => {
    debug(`Load Ats Routes between ${minCoordinates.latitude}/${minCoordinates.longitude} and ${maxCoordinates.latitude}/${maxCoordinates.longitude}`);
    context.atsRoutes = []
    if (!!!minCoordinates || !!!maxCoordinates) {
        return context.atsRoutes
    }
    const data = fs.readFileSync(path.join(__dirname, '../../data/1712/ATS.txt'), 'utf8')
    if (data) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (mode == 1 && words.length > 8) {
                ...
                step.heading = new Bearing(parseInt(words[8]))
