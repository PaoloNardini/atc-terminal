...
const loadScenario = async ( context: Context, name: string): Promise<Context> => {
    debug( `Load scenario`)
...
const loadAirport = async (context: Context, icao: string): Promise<Context> => {
    ...
    const data = fs.readFileSync(path.join(__dirname, '../../data/1712/Airports.txt'), 'utf8')
    if (data) {
