...
export const SocketFactory = (canvas: Canvas): SocketFactoryInterface => {
  ...
  return {
    sendMessage: (msgType: string, payload: any) => {
      ...
      console.log(
        `Send message of type ${msgType} with payload ${util.inspect(payload)}`
      )
      socket.emit(msgType, payload)
    },
    handleMessages: (msgType: string, payload: any) => {
      console.log(
        `received message of type ${msgType} with payload ${util.inspect(
          payload
        )}`
      )
...
export const handleGeneralMessage = async (
  msgType: string,
  payload: any,
  canvas: Canvas
): Promise<void> => {
  void canvas
  console.log(
    `received message of type ${msgType} with payload ${util.inspect(payload)}`
  )
}
...
export const handleScenarioMessage = async (
  ...
  console.log(
    `received scenario message of type ${msgType} with payload ${util.inspect(
      payload
    )}`
  )
  if (msgType == SocketMsgType.MSG_SCENARIO && payload.type == 'SCENARIO') {
    ...
    loadWaypoints(payload.scenario.waypoints, canvas)
  }
}

export const handlePlanesMessage = async (
  msgType: SocketMsgType,
  payload: any,
  canvas: Canvas
): Promise<void> => {
  console.log(
    `received planes message of type ${msgType} with payload ${util.inspect(
      payload,
      true,
      3
    )}`
